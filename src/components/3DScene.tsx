...
const ThreeDScene: React.FC<ThreeDSceneProps> = ({
  ...
  // Helper function to create baseplate geometry from config when refs are not available
  const createBaseplateGeometryFromConfig = useCallback((config: BasePlateConfig): THREE.BufferGeometry | null => {
    try {
  ...
  // Handle fixture export with manifold CSG union
  React.useEffect(() => {
    const handleExportFixture = async (e: CustomEvent) => {
      ...
      try {
        // Dispatch export-progress events for loading indicator
        ...
        // === COLLECT INDIVIDUAL COMPONENT GEOMETRIES ===
        // Instead of using the merged (non-manifold) geometry, collect individual
        // component geometries so Manifold3D can do a proper boolean union
        const geometriesToUnion: THREE.BufferGeometry[] = [];
        
        // 1. Add baseplate geometry (with holes if available)
        const isMultiSection = basePlate?.type === 'multi-section';
        
        if (baseplateWithHoles) {
          console.log('[Export] Adding baseplateWithHoles geometry');
          geometriesToUnion.push(baseplateWithHoles);
        } else if (isMultiSection && multiSectionBasePlateGroupRef.current) {
          // For multi-section, collect geometries from all children
          console.log('[Export] Adding multi-section baseplate geometries');
          multiSectionBasePlateGroupRef.current.traverse((child) => {
            if (child instanceof THREE.Mesh && child.geometry) {
              geometriesToUnion.push(child.geometry);
            }
          });
        } else if (basePlateMeshRef.current?.geometry) {
          // Baseplate mesh is rendered - use its geometry directly
          console.log('[Export] Adding baseplate geometry from ref');
          geometriesToUnion.push(basePlateMeshRef.current.geometry);
        } else if (originalBaseplateGeoRef.current) {
          // Baseplate mesh is not rendered (hidden by mergedFixtureMesh) but we have cached geometry
          console.log('[Export] Adding baseplate geometry from originalBaseplateGeoRef');
          geometriesToUnion.push(originalBaseplateGeoRef.current);
        } else if (basePlate) {
          // Last resort: try to create baseplate geometry from config
          // This handles the case where no holes were ever created
          console.log('[Export] Creating baseplate geometry from config');
          const baseplateGeo = createBaseplateGeometryFromConfig(basePlate);
          if (baseplateGeo) {
            geometriesToUnion.push(baseplateGeo);
          } else {
            console.warn('[Export] Could not create baseplate geometry from config');
          }
        }
        
        // Separate collection for support geometries (may overlap and need CSG union)
        const supportGeometries: THREE.BufferGeometry[] = [];
        
        // 2. Collect all support geometries (from modifiedSupportGeometries after cavity cut)
        if (modifiedSupportGeometries.size > 0) {
          ...
          modifiedSupportGeometries.forEach((geom, supportId) => {
            if (geom) {
              supportGeometries.push(geom);
            }
          });
        }
        
        // 2.5. Build clamp support geometries with cutouts (holes)
        // These are NOT in modifiedSupportGeometries (which only contains regular supports)
        // We need to build them fresh and apply cutouts CSG for the holes
        if (placedClamps.length > 0) {
          console.log(`[Export] Building ${placedClamps.length} clamp support geometries with cutouts`);
          emitProgress('preparing', 6, `Building ${placedClamps.length} clamp support geometries...`);
          ...
          for (const clamp of placedClamps) {
            ...
            if (!clampData?.supportInfo || !supportInfo) {
              console.log(`[Export] Skipping clamp ${clamp.id} - no support info`);
              continue;
            }
            ...
            // Calculate support height (same as useCavityOperations)
            // baseTopY is the world Y position of the baseplate's top surface
            ...
            if (!clampSupportGeometry) {
              console.warn(`[Export] Failed to build clamp support geometry for ${clamp.id}`);
              continue;
            }
            ...
            if (clampData.fixtureCutoutsGeometry && clampData.fixturePointTopCenter) {
              console.log(`[Export] Applying cutouts CSG for clamp ${clamp.id}`);
              emitProgress('preparing', 7, `Applying cutouts for clamp ${clamp.id}...`);
              
              // Clone and transform cutouts geometry to match support position (at origin)
              // The cutouts need to be positioned relative to the fixture point
              ...
              try {
                // Perform CSG subtraction to create holes in the support
                const csgResult = await performClampCSGInWorker(
                  clampSupportGeometry,
                  cutoutsClone,
                  (progress) => {
                    emitProgress('preparing', 7, `CSG for clamp ${clamp.id}: ${progress}%`);
                  }
                );
                
                if (csgResult) {
                  clampSupportGeometry = csgResult;
                  console.log(`[Export] Cutouts CSG completed for clamp ${clamp.id}`);
                }
              } catch (err) {
                console.warn(`[Export] Cutouts CSG failed for clamp ${clamp.id}, using support without holes:`, err);
              }
            }
            
            // Transform to world position (same logic as ClampSupportMesh)
            // Position Y is baseTopY (the top surface of the baseplate in world coordinates)
            ...
            supportGeometries.push(clampSupportGeometry);
            console.log(`[Export] Added clamp support for ${clamp.id}, height: ${supportHeight.toFixed(2)}mm`);
          }
        }
        
        // 2.6. If we have multiple supports, union them together to remove internal faces
        // This handles overlapping supports and supports overlapping with clamp supports
        if (supportGeometries.length > 1) {
          console.log(`[Export] Performing CSG union on ${supportGeometries.length} supports to remove internal faces...`);
          emitProgress('manifold', 10, `CSG union on ${supportGeometries.length} supports...`);
          
          try {
            const supportGeomsForWorker = supportGeometries.map((geom, idx) => ({
              id: `support-${idx}`,
              geometry: geom
            }));
            
            const supportUnionResult = await performRealCSGUnionInWorker(
              supportGeomsForWorker,
              undefined,
              (current, total, stage) => {
                const progress = 10 + ((current / total) * 20); // 10-30%
                emitProgress('manifold', progress, `Support union: ${stage} (${current}/${total})`);
              }
            );
            
            if (supportUnionResult) {
              console.log('[Export] Support CSG union succeeded - internal faces removed');
              geometriesToUnion.push(supportUnionResult);
            } else {
              console.warn('[Export] Support CSG union returned null, adding supports individually');
              supportGeometries.forEach(g => geometriesToUnion.push(g));
            }
          } catch (supportUnionError) {
            console.error('[Export] Support CSG union failed:', supportUnionError);
            console.warn('[Export] Adding supports individually without union');
            supportGeometries.forEach(g => geometriesToUnion.push(g));
          }
        } else if (supportGeometries.length === 1) {
          // Only one support, no need for union
          geometriesToUnion.push(supportGeometries[0]);
        }
        
        // 3. Build and add label geometries
        const currentLabels = labelsRef.current || [];
        if (currentLabels.length > 0) {
          console.log(`[Export] Building ${currentLabels.length} label geometries`);
          emitProgress('preparing', 35, `Building ${currentLabels.length} label geometries...`);
          
          for (const label of currentLabels) {
            try {
              const labelGeometry = await buildLabelGeometry(label);
              if (labelGeometry) {
                geometriesToUnion.push(labelGeometry);
              }
            } catch (err) {
        ...
        console.log(`[Export] Total geometries to merge: ${geometriesToUnion.length}`);
        
        // If we have no geometries to union, fall back to merged geometry
        if (geometriesToUnion.length === 0) {
          console.warn('[Export] No component geometries found, using merged geometry');
          geometriesToUnion.push(mergedFixtureMesh.geometry);
        }
        
        // === FINAL MERGE: Simple concatenation + vertex welding ===
        // Supports have already been CSG-unioned, so remaining components won't have internal faces
        emitProgress('manifold', 40, 'Merging final geometries...');
        ...
        try {
          ...
          const normalizedGeometries = geometriesToUnion.map(geom => {
            // Convert to non-indexed if necessary
            const nonIndexed = geom.index ? geom.toNonIndexed() : geom.clone();
            // Remove UVs if present (not needed for STL)
          ...
          emitProgress('manifold', 50, `Merging ${normalizedGeometries.length} geometries...`);
          
          // Merge all geometries
          const merged = mergeGeometries(normalizedGeometries, false);
          
          if (merged) {
            emitProgress('manifold', 60, 'Welding vertices...');
            
            // Weld duplicate vertices at seams for watertight mesh
            // Use small tolerance for precision
            const welded = mergeVertices(merged, 0.001);
            welded.computeVertexNormals();
            
            exportGeometry = welded;
          ...
          console.error('[Export] Final merge failed:', mergeError);
        }
        
        // === FALLBACK: Full CSG UNION (if fast merge failed) ===
        if (!exportGeometry) {
          console.log('[Export] Falling back to full CSG union...');
          emitProgress('manifold', 70, 'Trying full CSG union (fallback)...');
          
          try {
            const geometriesForWorker = geometriesToUnion.map((geom, idx) => ({
              id: `export-part-${idx}`,
              geometry: geom
            }));
            
            const csgProgressCallback = (current: number, total: number, stage: string) => {
              const progress = 70 + ((current / total) * 20); // 70-90%
              emitProgress('manifold', progress, `CSG Union: ${stage} (${current}/${total})`);
            };
            
            const csgResult = await performRealCSGUnionInWorker(
              geometriesForWorker,
              undefined,
              csgProgressCallback
            );
            ...
            if (csgResult) {
              ...
              exportGeometry = csgResult;
            }
          } catch (csgError) {
            console.error('[Export] CSG union failed:', csgError);
          }
        }
        
        // === LAST RESORT: Use pre-computed merged geometry ===
        if (!exportGeometry) {
          console.warn('[Export] All methods failed, using pre-computed merged geometry...');
          emitProgress('manifold', 90, 'Using cached geometry...');
          exportGeometry = mergedFixtureMesh.geometry;
        }
        
        emitProgress('exporting', 95, 'Generating STL file...');
        
        // Create a temporary mesh for STL export
        const exportMesh = new THREE.Mesh(exportGeometry, mergedFixtureMesh.material);
        exportMesh.matrix.copy(mergedFixtureMesh.matrix);
        exportMesh.matrixAutoUpdate = false;
        ...
        if (isMultiSection && config.splitParts && sections.length > 1) {
          ...
          for (let i = 0; i < sections.length; i++) {
            ...
            emitProgress('exporting', 85 + ((i / sections.length) * 10), `Exporting ${filename}...`);
        ...
        // Clean up any geometry we created (don't dispose original merged geometry)
        if (exportGeometry && exportGeometry !== mergedFixtureMesh.geometry) {
  ...
  }, [mergedFixtureMesh, basePlate, baseplateWithHoles, modifiedSupportGeometries, labelsRef, basePlateMeshRef, multiSectionBasePlateGroupRef, placedClamps, clampSupportInfos, loadedClampDataRef, baseTopY]);
